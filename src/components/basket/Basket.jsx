...
const Basket = () => {
  ...
  useEffect(() => {
    if (didMount && firebase.auth.currentUser && basket.length !== 0) {
      firebase.saveBasketItems(basket, firebase.auth.currentUser.uid)
        .then(() => {
          console.log('Работа добавлена в корзину');
        })
        .catch((e) => {
          console.log(e);
        });
  ...
  return user && user.role === 'ADMIN' ? null : (
    <Boundary>
      ...
      >
        <p className="text-center">Вы должны войти в систему чтобы сделать чекаут</p>
        <br />
        <div className="d-flex-center">
          ...
          <button
            ...
            type="button"
          >
            Войти в систему чтобы сделать чекаут
          </button>
        </div>
      </Modal>
        ...
        <div className="basket-list">
          ...
          {basket.length <= 0 && (
            <div className="basket-empty">
              <h5 className="basket-empty-msg">Ваша корзина пустая</h5>
            </div>
          )}
